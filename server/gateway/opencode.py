...
def proxy(handler: BaseHTTPRequestHandler, method: str, path: str) -> None:
    ...
    try:
        ...
        upstream = requests.request(
            method if not translate else "POST",
            url,
            headers=headers,
        ...
        if is_stream:
            ...
            handler.end_headers()
            if method != "HEAD":
                raw_chunks = upstream.iter_content(chunk_size=4096)
                ...
                if translate:
                    ...
                    chunks = iter_anthropic_sse(_tee(raw_chunks), model)
